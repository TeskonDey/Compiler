    ...
    def generate(self, node):
        if node['type'] == 'INTEGER':
            ...
            if node['op'] == 'SQUARE':
                ...
                self.add_instruction(f'SQUARE {result_reg}, {right_reg}', f'Square {right_reg}')
                return result_reg, right_val ** 2
            ...
            if node['op'] == '+':
                ...
                self.add_instruction(f'ADD {result_reg}, {left_reg}, {right_reg}', f'Add {left_reg} and {right_reg}')
            elif node['op'] == '-':
                result = left_val - right_val
                self.add_instruction(f'SUB {result_reg}, {left_reg}, {right_reg}', f'Subtract {right_reg} from {left_reg}')
            elif node['op'] == '*':
                result = left_val * right_val
                self.add_instruction(f'MUL {result_reg}, {left_reg}, {right_reg}', f'Multiply {left_reg} by {right_reg}')
            elif node['op'] == '/':
                result = left_val / right_val
                self.add_instruction(f'DIV {result_reg}, {left_reg}, {right_reg}', f'Divide {left_reg} by {right_reg}')
